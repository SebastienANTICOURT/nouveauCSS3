...
function createCard(title, imageUrl) {
  ...
  const cardBody = document.createElement("div");
    ...
    document.body.cards.card.appendChild(cardBody);

    const cardTitle = document.createElement("h2");
    cardTitle.classList.add("cardTitle");
    cardTitle.innerHTML = "title";
    document.body.cards.card.cardBody.appendChild(cardTitle);

    const cardButton =document.createElement('button');
    cardButton.classList.add("card-button");
    cardButton.innerHTML = "Adopt Now";
    document.body.cardBody.appendChild(cardButton);
}

animalsToAdopt.forEach(createCard => console.log(createCard) );
